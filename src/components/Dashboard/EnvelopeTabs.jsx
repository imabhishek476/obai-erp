...
const EnvelopeTabs = ({ countData, timeFilter,setLoading }) => {
    ...
    const getEnvelopeList = async (filter, envelopePage, timeFilter) => {
        ...
        const res = await getcompletedDocList(filterBy, envelopePage?.currentPage, envelopePage?.size, timeFilter)
        console.log(res)
        if (res) {
            setEnvelopeData(res)
            setEnvelopePage({
    ...
    const handleNext = async (skip) => {
        ...
        const response = await getcompletedDocList(selected, envelopePage?.currentPage + 1 , envelopePage?.size,timeFilter || "0");
        if (response) {
